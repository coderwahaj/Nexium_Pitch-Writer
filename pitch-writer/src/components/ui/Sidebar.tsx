...
import { Zap, Info, LogIn, LogOut, Rocket, Clock } from "lucide-react";
...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    ...(user
      ? [
          { href: "/dashboard", label: "New Pitch", icon: Zap },
          { href: "/mypitches", label: "My Pitches", icon: Clock },
  ...
  return (
    <div className="hidden md:block fixed top-0 left-0 h-full w-64 bg-[#12131f]">
      <aside className="fixed top-0 left-0 h-full w-64 p-4 z-50 hidden lg:flex flex-col glass-container  bg-black/30">
        <Link
          href={user ? "/dashboard" : "/"}
          className="flex items-center gap-2 mb-10 p-2"
        >
